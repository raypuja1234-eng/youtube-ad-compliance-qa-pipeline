...
class VideoIndexerService:
    def __init__(self):
        ...
        self.vi_name = os.getenv("AZURE_VI_NAME","brand-yt-project-11")
    ...
    def get_account_token(self,arm_access_token):
        '''
        Exchanges the ARM token for video indexer acoount team
        '''
        url = (
            f"https://management.azure.com/subscriptions/{self.subscription_id}"
            f"/resourceGroups/{self.resource_group}"
            f"/providers/Microsoft.VideoIndexer/accounts/{self.vi_name}"
            f"/generateAccessToken?api-version=2024-12-01"
    ...
    def download_youtube_video(self,url,output_path="temp_video.mp4"):
        '''
        downloads the youtube video to a local files
        '''
        logger.info(f"Downloading Youtube video: {url}")

        ydl_opts= {
            "format" : 'best',
            'outtmpl' : output_path,#output templates
            'quiet' : False,
            'no_warnings' : False,
